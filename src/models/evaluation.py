...
def rolling_origin_arima_evaluation(
    ...
    with warnings.catch_warnings():
        ...
        for idx, value in test.items():
            ...
            new_obs = pd.Series([value], index=[idx], name=series.name)
            try:
                current_res = current_res.append(new_obs, refit=False)
            except Exception:
                # Fallback: refit if append fails (e.g., due to index issues)
                updated_history = pd.concat([train, test.loc[:idx]])
                current_res = run_arima(updated_history, base_order)["model"]
